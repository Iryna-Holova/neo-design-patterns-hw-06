...
[2026-06-16 03:50:27] ПРИВІТ! ЯК СПРАВИ?
[RateLimit] skipped

# Після очікування 1 секунди:
[2026-06-16 03:50:28] ЦЕ ПОВІДОМЛЕННЯ ВЖЕ ПРОЙДЕ, БО МИ ПОЧЕКАЛИ 1 СЕКУНДУ
```
...
1. **Декоратори** (TypeScript experimental decorators):
   - Модифікують поведінку методу `send` без зміни його коду
   - Застосовуються через синтаксис `@`
   - Використовуються для трансформації тексту повідомлення перед відправкою

2. **Proxy** (замісник через клас-обгортку):
   - Контролює частоту викликів `send`
   - Блокує повторні повідомлення, якщо вони надходять швидше, ніж `intervalMs`
   - Працює через фабричну функцію `createRateLimitProxy`, не змінюючи основний сервіс
